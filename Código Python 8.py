...
df_iptu = df[['Regiões Administrativas', 'Média de IPTU_Residencial']]
...
# Obtendo os valores mínimo e máximo de Média de IPTU_Residencial min_iptu = df_iptu['Média de IPTU_Residencial'].min()
max_iptu = df_iptu['Média de IPTU_Residencial'].max()

# Criando o mapa coroplético fig = px.choropleth_mapbox(
df_iptu, geojson=geo_uf,
locations='Regiões Administrativas', featureidkey='properties.nomera', color='Média de IPTU_Residencial', color_continuous_scale="Turbo", range_color=(min_iptu, max_iptu), mapbox_style='open-street-map',
zoom=10, # Ajuste para um nível de zoom apropriado labels={'Média de IPTU_Residencial': 'IPTU Médio Residencial
(R$)', 'nomera': 'Região Administrativa'}, center={"lat": -22.908333, "lon": -43.196388},
title='Mapa IPTU Residencial por RA', width=1000,
height=450
)
